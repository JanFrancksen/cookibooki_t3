...
const filteredRecipeList = recipes.filter((val) =>{
  if (searchTerm==""){
    return val
...
<RecipeItem key={recipe.id} image={recipe.img} title={recipe.title} description={recipe.content} />
  ...
  return (
    <div>
      ...
      <div className='z-10'>
        <input
         type="text"onChange={(event) => {setSearchTerm(event.target.value)}} placeholder="Search.." className='py-2 px-4 rounded-full mr-4' /> <button className='bg-cb_green py-2 px-4 rounded-full'>suchen</button>
        </div>
